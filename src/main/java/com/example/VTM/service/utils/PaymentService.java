...
//            options.put("description", "BMG Jewellery Pvt Ltd.");
//
...
public class PaymentService {
    ...
    // Create Order (skip Razorpay)
    public Map<String, Object> createOrder(PaymentRequest paymentRequest) {
        Map<String, Object> response = new HashMap<>();
        ...
        try {
            ...
            String sql = "INSERT INTO AppPayment_record " +
                    "(order_id, name, contact, REGNO, GROUPCODE, amount, status, created_at) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public Map<String, Object> cancelOrder(String orderId) {
        ...
        try {
            String updateSql = "UPDATE AppPayment_record SET status = ? WHERE order_id = ?";
            int updated = jdbcTemplate.update(updateSql, "CANCEL", orderId);
            ...
            if (updated > 0) {
                ...
                response.put("error", "Order not found for ID: " + orderId);
            }
        } catch (Exception e) {
            e.printStackTrace();
